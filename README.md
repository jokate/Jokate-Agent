...
- **Pre-approved shell:** with Bash, read-only inspection commands (ls, cat, head, tail, wc, grep, rg, find, git log/show/diff/status) are always allowed, so a chained `cat a; ls b` does not stop at "requires approval".
